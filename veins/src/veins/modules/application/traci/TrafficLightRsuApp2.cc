...
#include "veins/modules/application/traci/TrafficLightRsuApp2.h"

using namespace veins;

Define_Module(veins::TrafficLightRsuApp2);

void TrafficLightRsuApp2::initialize(int stage)
{
...
void TrafficLightRsuApp2::finish()
{
    DemoBaseApplLayer::finish();
    // statistics recording goes here
}

void TrafficLightRsuApp2::onBSM(DemoSafetyMessage* bsm)
{
    ...
    DemoSafetyMessage* wsmBeacon = check_and_cast<DemoSafetyMessage*>(bsm);
            ...
            //EV << "idvehicle" <<  idVehicle << std::endl;
                ...
                if (wsmBeacon->getPsid() == 0) {
                    if ((wsmBeacon->getSenderPos().distance(curPosition)) < 25) {
                        if (!phaseMsg->isScheduled()) {
                            traci->trafficlight(trafficLightId).setProgram("p1");
                            scheduleAt(simTime()+15,phaseMsg);
                        }
                    }
            }
		if (wsmBeacon->getPsid() == 1) {
                                    if ((wsmBeacon->getSenderPos().distance(curPosition)) < 25) {	
                                        if (!phaseMsg->isScheduled()) {
                                            traci->trafficlight(trafficLightId).setProgram("p1");
                                            scheduleAt(simTime()+15,phaseMsg);
...
void TrafficLightRsuApp2::onWSM(BaseFrame1609_4* wsm)
{
    // Your application has received a data message from another car or RSU
    // code for handling the message goes here, see TraciDemo11p.cc for examples
}

void TrafficLightRsuApp2::onWSA(DemoServiceAdvertisment* wsa)
{
    // Your application has received a service advertisement from another car or RSU
    // code for handling the message goes here, see TraciDemo11p.cc for examples
}

void TrafficLightRsuApp2::handleSelfMsg(cMessage* msg)
{
...
void TrafficLightRsuApp2::handlePositionUpdate(cObject* obj)
{
